...
window.HInclude.HIncludeElement = window.HIncludeElement = (function () {
  ...
  function showContent(element, req) {
    ...
    if (req.status === 200 || (req.status === 304 && !element.isRefreshing)) {
      ...
      var node = element.extractFragment.call(
        element,
        container,
        fragment,
        req
  ...
  function showBufferedContent() {
    while (buffer.length > 0) {
      ...
      try {
        ...
        // rethrow error without stopping the loop
        setTimeout(function () {
          throw error;
        });
      }
  ...
  var checkRecursion = function (element) {
    ...
    while (elementToCheck.parentNode) {
      if (elementToCheck.nodeName === TAGNAME) {
        if (
          element.getAttribute("src") === elementToCheck.getAttribute("src")
        ) {
          throw new Error("Recursion not allowed");
        }
      }
  ...
  var getUrl = function (element) {
    ...
    var whenCondition =
      whenFalseUrl && !element.conditionalInclusion.call(element, "when");
    var mediaCondition = !element.conditionalInclusion.call(element, "media");

    return getConditionalUrl(
      element,
      whenCondition,
      mediaCondition,
      altSrcInclude
  ...
  var getConditionalUrl = function (
    element,
    whenCondition,
    mediaCondition,
    altSrcInclude
